...
waitForUser("Waiting for user to APPLY threshold. Press Okay once done.");


//This section trims of extra blebs and generates an ROI for each egg
run("Watershed");
waitForUser("Press Okay if masks look acceptable. Small particles will be ignored. You can use eraser tool now if necessary.");
...
for (i = 0; i < roicount; i++) { // outer 'for' loop iterating through eggs
	...
	run("Duplicate...", "title="+i); //creates the new image with all eggs
	...
	}// closing inner for loop, an image is should now exist with only one egg (i).

	
	roiManager("deselect") //deselcting any residual ROIs
	roiManager("delete") // clearing all ROIs
	selectWindow("C"+maskingchannel+"-"+title); // returning to original mask of all eggs
	//waitForUser("pausing after first loop is complete");
	
}// closing outer for loop, which performs the inner loop for each egg adn then returns to original mask of all eggs
